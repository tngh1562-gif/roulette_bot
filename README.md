...
| /내전등록버튼 | 채널에 내전 참가 등록 버튼 메시지 생성 |

## 사용 예시
내전 참가 등록 버튼 만들기:
→ 내전사이트 `디스코드 관리` 탭에서 등록 버튼 ON/OFF와 버튼 문구 설정
→ 채널별로 `최근 메시지 아래 버튼 생성` ON/OFF 설정 가능
→ `버튼만 생성`을 ON으로 두면 안내문 없이 버튼 메시지만 생성
→ `일반 글 + 버튼`을 ON으로 두면 사이트의 큰 글쓰기 박스 내용을 디스코드 마크다운 그대로 보내고 아래에 버튼 생성
...
디스코드에서 내전 참가 양식 등록:
→ `/내전등록`
→ 팝업창에 롤 닉네임, 치지직 닉네임, 티어, 주 포지션, 부 포지션 1/2 입력
→ 부 포지션은 `미드, 원딜`처럼 두 개까지 한 칸에 입력 가능
